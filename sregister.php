...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (sizeof($student) > 0) {
        ...
        $Sfirst_name = mysqli_query($db, "SELECT Sfirst_name FROM STUDENT WHERE Semail = '$email'");
        $name_check = mysqli_fetch_assoc($Sfirst_name);

        if ($name_check['Sfirst_name'] == "null") {
...
//                    //assigning 2 reviewers with the least amount abstracts
//                    $lowest_faculty = $mysqli_query($db, "SELECT Fid, count(*) as cnt FROM REVIEWS GROUP BY Fid ORDER BY cnt ASC LIMIT 2");
//                    $index = 0;
//                    while($row = mysqli_fetch_assoc($lowest_faculty)){
//                        $new_array[$index] = $row;
//                        $index++;
//                    }
//                    $sql2 = "INSERT INTO REVIEWS"
                    $mentor_id = mysqli_real_escape_string($db, $_POST['Fid']);
                    $sql2 = "INSERT INTO HAS VALUES('$student_id', '$mentor_id')";
